...
POSSIBLE_DANCER_CHARS = [chr(i) for i in range(65, 91) if chr(i) not in RESERVED_CHARS]
...
class DancerSprite(prefab_sprites.MazeWalker):
    ...
    def __init__(self, corner, position, character, motion, color, shape, value=0.0):
        super(DancerSprite, self).__init__(corner, position, character, impassable="#")
        self.motion = motion
...
def make_game(dancers_and_properties, dance_delay=16):
    ...
    this_game = ascii_art.ascii_art_to_game(
        ...
        what_lies_beneath=" ",
        sprites=sprites,
        update_schedule=[[AGENT_CHAR], dance_order],
    )
...
def main(argv):
    ...
    bg_colours = {c: (0, 0, 0) for c in RESERVED_CHARS + POSSIBLE_DANCER_CHARS[1:8]}
