...
class Numpad2DDiscrete(Numpad):
    ...
    def __init__(self, config: Config = Config()):
        ...
        if not self.flatten_state:
            self.observation_space = spaces.Box(
                low=0,
                high=1,
                shape=(self.NUM_CHANNELS, config.numpad_size, config.numpad_size),
                dtype=np.int64,
            )
        else:
            self.observation_space = spaces.Box(
        ...
        if config.ball_start and config.seq:
            self.state[self.BALL_CHANNEL, self.ball_pos[0], self.ball_pos[1]] = (
                self.NO_BALL
            )
            self.state[
                self.BALL_CHANNEL, config.ball_start[0], config.ball_start[1]
            ] = self.BALL
    ...
    def step(self, action):
        ...
        self.state[self.BALL_CHANNEL, self.ball_pos[0], self.ball_pos[1]] = self.BALL
        ...
        if self.ball_pos == seq_next:
            self.state[self.LIGHTS_CHANNEL, self.ball_pos[0], self.ball_pos[1]] = (
                self.LIGHT_ON
            )
            ...
            if self.seq_max_cnt == self.seq_len - 1:
                ...
                self.seq_cnt = 0
                self.seq_max_cnt = -1
                return tmp_state, reward, self.done, False, info

        # if self.done:
        #    print(f"Stepped on tiles: {self.stepped_on}")
        if self.flatten_state:
            return self.state.flatten(), reward, self.done, False, info
        else:
    ...
    def reset(self, seed=None, **kwargs):
        ...
        info = {}
        self.state = np.tile(
            self.LIGHT_OFF, (self.NUM_CHANNELS, self.numpad_size, self.numpad_size)
        )
        self.t = 0
        self.done = False
