...
class Numpad2DContinuous(Numpad):
    ...
    def __init__(self, config: Config = Config()):
        ...
        self.action_space = spaces.Box(
            low=self.ACTION_LOW, high=self.ACTION_HIGH, shape=(2,), dtype=float
        )
        ...
        self.img_size = (
            config.numpad_size * config.tile_size
            + (2 * config.numpad_size) * config.spacing_size
        )  # pixels of all tiles + pixels of all spacing
        ...
        if self.observation_mode == "rgb_array":
            self.observation_space = spaces.Box(
                low=0, high=1, shape=(3, self.img_size, self.img_size), dtype=float
            )
        elif self.observation_mode == "greyscale_array":
            self.observation_space = spaces.Box(
                low=0, high=1, shape=(1, self.img_size, self.img_size), dtype=float
            )
    ...
    def __seq_num_to_idx(self, seq_num):
        return np.array([int(seq_num / self.numpad_size), seq_num % self.numpad_size])
    ...
    def __idx_to_pos(self, idx):
        ...
        return (
            idx * (self.img_size / self.numpad_size)
    ...
    def __pos_to_seq_num(self, pos):
        ...
        tile_bounds_left, tile_bounds_right = (
            center_of_tile - self.tile_size / 2,
            center_of_tile + self.tile_size / 2,
        )

        # check if ball is on tile, and return sequence number if true, else return None
        if (
            pos[0] > tile_bounds_left[0]
            and pos[1] > tile_bounds_left[1]
            and pos[0] < tile_bounds_right[0]
            and pos[1] < tile_bounds_right[1]
        ):
    ...
    def __draw_square(self, rgb_img, center, size, color):
        ...
        rgb_img[:, start[0] : end[0] + 1, start[1] : end[1] + 1] = color.reshape(
            -1, 1, 1
        )
    ...
    def __generate_observation(self, copy=False):
        if (
            ...
            or self.observation_mode == "greyscale_array"
        ):
    ...
    def step(self, action):
        ...
        if self.done:
            ...
            observation = np.concatenate([self.ball_pos / self.img_size, self.lights])
        ...
        self.ball_velocity += np.clip(
            action * self.acc_scalar, -self.max_velocity, self.max_velocity
        )
        ...
        curr_seq_num = self.__pos_to_seq_num(self.ball_pos)  # == None if on spacing
    ...
    def render(
        ...
        if mode == "array":
            if self.observation_mode == "rgb_array":
                ...
                return np.concatenate([self.ball_pos / self.img_size, self.lights])
        elif mode == "print":
            ...
            im = Image.fromarray(
                np.rollaxis(arr.transpose((0, 2, 1)), 0, 3), mode=img_mode
            )
            ...
            if (self.t + 1) == self.t_max:
                ...
                try:
                    ...
                    print("gifsicle not available - unable to optimize gif size")
        else:
            modes_str = "".join(
                [
                    f"\n\t{available_mode}"
