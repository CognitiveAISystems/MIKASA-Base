...
class PrevActionAndReward(gym.Wrapper):
    ...
    def __init__(self, env, padding=0.0):
        assert isinstance(
            env.action_space, spaces.Discrete
        ), "This implementation works only with discrete action spaces"
        super().__init__(env)
...
class AddMemory(gym.Wrapper):
    ...
    def _add_done_to_vector_obs(self, obs, done):
        return dict(observation=obs, done=self._preproc_done(done))
    ...
    def _preproc_obs(self, obs, done):
        ...
        return {
            k: torch.as_tensor(v).unsqueeze(1).to(self.device) for k, v in obs.items()
        }
