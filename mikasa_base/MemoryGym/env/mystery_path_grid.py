...
from .pygame_assets import MysteryPath
...
class GridMysteryPathEnv(CustomEnv):
    ...
    def _build_debug_surface(self):
        ...
        surface.fill(0)
        self.mystery_path.draw_to_surface(
            surface, self.tile_dim, True, True, True, True
        )
    ...
    def reset(self, seed=None, return_info=True, options=None):
        ...
        cardinal_origin = self.np_random.choice(
            self.reset_params["cardinal_origin_choice"]
        )
        ...
        self.mystery_path = MysteryPath(
            self.grid_dim, self.grid_dim, self.start, self.end, self.np_random
        )
        ...
        start_pos = (
            self.start[0] * self.tile_dim + int(25 * SCALE),
            self.start[1] * self.tile_dim + int(25 * SCALE),
        )  # TODO depend on agent radius and not hard coded int(25 * SCALE)
        self.normalized_agent_position = self._normalize_agent_position(start_pos)
        self.agent = GridCharacterController(
            SCALE,
            self.normalized_agent_position,
            self.mystery_path.to_grid(self.tile_dim),
            rotation,
        )
        self.rotated_agent_surface, self.rotated_agent_rect = (
            self.agent.get_rotated_sprite(rotation)
        )
    ...
    def step(self, action):
        ...
        if self.normalized_agent_position == self.end:
            ...
            for node in self.mystery_path.path:
                if self.normalized_agent_position == (node.x, node.y):
                    on_path = True
                    if (
                        not node.reward_visited
                        and not (node.x, node.y) == self.start
                        and not (node.x, node.y) == self.end
                    ):
        ...
        self._draw_surfaces(
            [
                (self.path_surface, (0, 0)),
                (self.rotated_agent_surface, self.rotated_agent_rect),
                (self.fall_off_surface, self.fall_off_rect),
            ]
        )
