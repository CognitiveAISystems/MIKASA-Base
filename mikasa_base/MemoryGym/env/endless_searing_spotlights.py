...
class EndlessSearingSpotlightsEnv(CustomEnv):
    ...
    def __init__(self, render_mode=None) -> None:
        ...
        self.blue_background_surface = get_tiled_background_surface(
            self.screen, self.screen_dim, (0, 0, 255), SCALE
        )
        self.red_background_surface = get_tiled_background_surface(
            self.screen, self.screen_dim, (255, 0, 0), SCALE
        )
        ...
        self.walkable_rect = pygame.Rect(
            0, 16 * SCALE, self.screen_dim, self.screen_dim - 16 * SCALE
        )
    ...
    def _step_spotlight_task(self):
            ...
            self.spotlights.append(
                Spotlight(
                    ...
                    self.np_random.integers(
                        self.reset_params["spot_min_radius"],
                        self.reset_params["spot_max_radius"] + 1,
                    ),
                    self.np_random.uniform(
                        self.reset_params["spot_min_speed"],
                        self.reset_params["spot_max_speed"],
                    ),
                    self.np_random,
        ...
        if spotlight_hit > 0:
            ...
            width = int(
                self.screen_dim
                // 2
                * (1 - self.current_agent_health / self.agent_health)
            )
            pygame.draw.rect(
                self.top_bar_surface, (255, 0, 0), (0, 0, width, 16 * SCALE)
            )
    ...
    def _spawn_coin(self):
        ...
        if self.coin is not None:
            self.grid_sampler.block_spawn_position(
                (self.coin.location[0], self.coin.location[1]), 28
            )
        spawn_pos = self.grid_sampler.sample(28)
        spawn_pos = (
            spawn_pos[0] + self.np_random.integers(2, 4),
            spawn_pos[1] + self.np_random.integers(2, 4),
        )
    ...
    def reset(self, seed=None, return_info=True, options=None):
        ...
        self.last_action = [
            0,
            0,
        ]  # The agent shall sense its last action to potentially infer its postion from its past actions
        rotation = self.np_random.choice([0, 45, 90, 135, 180, 225, 270, 315])
        self.agent = CharacterController(
            self.reset_params["agent_speed"], self.reset_params["agent_scale"], rotation
        )
        ...
        pygame.draw.rect(
            self.top_bar_surface,
            (0, 255, 0),
            (0, 0, self.quarter_width * 2, 16 * SCALE),
        )
        ...
        if self.reset_params["show_last_action"]:
            ...
            self.act_rect_0 = (
                self.quarter_width * 2,
                0,
                self.quarter_width,
                16 * SCALE,
            )
            self.act_rect_1 = (
                self.quarter_width * 3,
                0,
                self.quarter_width,
                16 * SCALE,
            )
            pygame.draw.rect(
                self.top_bar_surface, self.action_colors[0], self.act_rect_0
            )
            pygame.draw.rect(
                self.top_bar_surface, self.action_colors[0], self.act_rect_1
            )
        if self.reset_params["show_last_positive_reward"]:
            self.last_reward = 0.0
            if self.reset_params["show_last_action"]:
                self.coin_bar_rect = (
                    int(self.quarter_width * 2.75),
                    0,
                    int(self.quarter_width * 0.5),
                    16 * SCALE,
                )
            else:
                self.coin_bar_rect = (
                    int(self.quarter_width * 2),
                    0,
                    int(self.quarter_width * 2),
                    16 * SCALE,
                )
        ...
        surfaces = [
            (self.bg, (0, 0)),
            (self.spotlight_surface, (0, 0)),
            (self.top_bar_surface, (0, 0)),
        ]
        spot_surface_id = 1
        # Coin surface
        if (
            self.reset_params["coins_visible"]
            or self.coin_t < self.reset_params["coin_show_duration"]
        ):
        ...
        return vis_obs, {
            "ground_truth": np.concatenate(
                (np.asarray(self.agent.rect.center), coin_location)
            )
            / self.screen_dim
        }
    ...
    def step(self, action):
        ...
        self.rotated_agent_surface, self.rotated_agent_rect = self.agent.step(
            action, self.walkable_rect
        )

        # Render the last action of the agent
        pygame.draw.rect(
            self.top_bar_surface,
            self.action_colors[self.last_action[0]],
            self.act_rect_0,
        )
        pygame.draw.rect(
            self.top_bar_surface,
            self.action_colors[self.last_action[1]],
            self.act_rect_1,
        )
        ...
        if self.spotlight_surface.get_alpha() <= self.reset_params["light_threshold"]:
            if self.reset_params["light_dim_off_duration"] > 0:
                self.spotlight_surface.set_alpha(
                    self.spotlight_surface.get_alpha()
                    + int(255 / self.reset_params["light_dim_off_duration"])
                )
            else:
                self.spotlight_surface.set_alpha(self.reset_params["light_threshold"])
        ...
        self.coin_t += 1
        if (
            self.coin_t == self.reset_params["steps_per_coin"]
            and self.reset_params["coin_enabled"]
        ):
            done = True
        ...
        if self.reset_params["show_last_positive_reward"]:
            if self.last_reward > 0:
                pygame.draw.rect(
                    self.top_bar_surface, (255, 255, 0), self.coin_bar_rect
                )
            else:
        ...
        if self.reset_params["agent_visible"]:
            ...
            surfaces.insert(
                spot_surface_id, (self.rotated_agent_surface, self.rotated_agent_rect)
            )
            spot_surface_id += 1
        self._draw_surfaces(surfaces)
