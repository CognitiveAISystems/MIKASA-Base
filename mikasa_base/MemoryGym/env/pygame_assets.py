...
class GridPositionSampler:
    def __init__(self, world_dim) -> None:
        ...
        self.area = np.arange(0, self.grid_dim**2, dtype=np.int32).reshape(
...
class Spotlight:
    def __init__(self, dim, radius, speed, rng, has_border=False) -> None:
        ...
        self.target_location = center + Vector2(self.spawn_radius, 0).rotate(
            target_angle
        )
        # Calculate offset location
        self.offset_location = center + Vector2(self.spawn_radius, 0).rotate(
            offset_angle
        )
    ...
    def draw(self, surface):
        ...
        pygame.draw.circle(
            surface,
            (255, 0, 0),
            (int(self.current_location.x), int(self.current_location.y)),
            self.radius,
        )
        if self.has_border:
            pygame.draw.circle(
...
class Coin:
    ...
    def draw(self, surface):
        ...
        pygame.draw.circle(
            surface, (255, 165, 0), self.location, self.radius, int(2 * self.scale)
        )
...
class Command:
    ...
    def __init__(self, command_type, scale) -> None:
        ...
        if command_type == "stay":
            ...
            pygame.draw.line(
                self.surface, (255, 255, 255), (0, y), (x, y), width=line_width
            )
            ...
            y2 = 0
            pygame.draw.line(
                self.surface, (255, 255, 255), (x1, y1), (x2, y1), line_width
            )
            pygame.draw.line(
                self.surface, (255, 255, 255), (x2, y1), (y1, y2), line_width
            )
            pygame.draw.line(
                self.surface, (255, 255, 255), (x2, y1), (y1, x2), line_width
            )
            # Determine rotation
            angle = 0
...
class MortarTile:
    def __init__(self, dim, scale, global_position, surface_rect) -> None:
        ...
        self.local_position = (
            global_position[0] - surface_rect[0],
            global_position[1] - surface_rect[1],
        )
        self.normalized_pos = (
            self.local_position[0] // self.dim,
            self.local_position[1] // self.dim,
        )
        self.blue = (21, 43, 77)
        ...
        pygame.draw.rect(
            self.surface, self.light_blue, ((0, 0, dim, dim)), width=int(4 * scale)
        )
    ...
    def to_grid(self):
        ...
        for i in range(self.arena_size):
            for j in range(self.arena_size):
                ...
                grid[i].append(
                    GridPosition(
                        tile.global_position[0] + translate_x,
                        tile.global_position[1] + translate_y,
                        i,
                        j,
                    )
                )
...
def calc_max_episode_steps(
    command_count, show_duration, show_delay, execution_duration, execution_delay
):
...
class Node:
    ...
    def draw_to_surface(self, surface, tile_dim, color):
        ...
        if self.is_wall:
            color = (255, 0, 0)
        pygame.draw.rect(
            surface, color, (self.x * tile_dim, self.y * tile_dim, tile_dim, tile_dim)
        )


class EndlessMysteryPath:
    def __init__(
        self, num_columns, num_rows, tile_dim, rng, num_initial_segments
    ) -> None:
    ...
    def render_path(self):
        ...
        surface = pygame.Surface(
            (
                (self.num_columns * self.num_segments + self.num_segments) * tile_size,
    ...
    def add_path_segment(self):
        ...
        transition_node = Node(
            self.end_position[0]
            + self.num_segments
            + (self.num_segments - 1) * self.num_rows,
            self.end_position[1],
        )
...
class MysteryPath:
    def __init__(
        self, num_columns, num_rows, start_position, end_position, rng
    ) -> None:
                ...
                if i == 0 or i == num_columns - 1 or j == 0 or j == num_rows - 1:
                    ...
                    if i != start_position[0] or j != start_position[1]:
                        if i != end_position[0] or j != end_position[1]:
                            if (
                                node not in start_node.neighbors
                                and node not in end_node.neighbors
                            ):
                ...
                if current_node == end_node:
                    ...
                    for neighbor in current_node.neighbors:
                        if neighbor in closed_set or neighbor.is_wall:
                            continue
                        g = current_node.g_cost + rng.integers(
                            1, 9
                        )  # g_cost_noise[current_node.x, current_node.y]
    ...
    def to_grid(self, cell_dim):
        ...
        for i in range(size):
            for j in range(size):
                cells[i].append(
                    GridPosition(
                        cell_dim * i + cell_dim // 2, cell_dim * j + cell_dim // 2, i, j
                    )
                )
        return cells
