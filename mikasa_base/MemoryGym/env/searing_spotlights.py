...
class SearingSpotlightsEnv(CustomEnv):
    ...
    def _build_debug_surface(self):
        ...
        surfs = [
            ...
            (self.exit.surface, self.exit.rect),
            (coin_surface, (0, 0)),
        ]
    ...
    def reset(self, seed=None, return_info=True, options=None):
        ...
        pygame.draw.rect(
            self.top_bar_surface, (50, 50, 50), (0, 0, self.screen_dim, 16 * SCALE)
        )
        pygame.draw.rect(
        ...
        self.coins = []
        self.num_coins = (
            self.np_random.choice(self.reset_params["num_coins"])
            if len(self.reset_params["num_coins"]) > 0
            else 0
        )
        if self.num_coins > 0:
            self._spawn_coins()
