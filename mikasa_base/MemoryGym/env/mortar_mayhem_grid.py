...
from .character_controller import GridCharacterController
from .pygame_assets import Command, MortarArena, calc_max_episode_steps
...
class GridMortarMayhemEnv(CustomEnv):
    ...
    def process_reset_params(reset_params):
        ...
        if reset_params is not None:
            for k, v in reset_params.items():
                assert k in cloned_params.keys(), (
                    "Provided reset parameter ("
                    + str(k)
                    + ") is not valid. Check spelling."
                )
                cloned_params[k] = v
        assert (
            cloned_params["allowed_commands"] >= 4
            and cloned_params["allowed_commands"] <= 9
        )
    ...
    def __init__(self, render_mode=None) -> None:
        ...
        self.screen = pygame.display.set_mode(
            (self.screen_dim, self.screen_dim), pygame.NOFRAME
        )
        ...
        self.observation_space = spaces.Box(
            low=0, high=255, shape=[self.screen_dim, self.screen_dim, 3], dtype=np.uint8
        )
    ...
    def _build_debug_surface(self):
        ...
        if self._command_visualization:
            ...
            surfs.append(
                (
                    ...
                    (
                        (
                            (self.screen_dim // 2) - command.rect_dim // 2,
                            (self.screen_dim // 2) - command.rect_dim // 2,
                        )
                    ),
                )
        ...
        target_tile = self.arena.tiles[int(self._target_pos[0])][
            int(self._target_pos[1])
        ]
        translation = (
            self.arena.rect.center[0]
            - self.arena.local_center[0]
            + self.arena.tile_dim // 2
        )
        pos = (
            target_tile.global_position[0] + translation,
            target_tile.global_position[1] + translation,
        )
        pygame.draw.circle(
            surface, (0, 255, 0), pos, self.arena.tile_dim // 2, int(8 * SCALE)
        )
    ...
    def _normalize_agent_position(self, agent_position):
        ...
        """
        return (
            (agent_position[0] - self.arena.rect[0]) // self.arena.tile_dim,
    ...
    def _generate_commands(self, start_pos):
        ...
        for i in range(self.num_commands):
            ...
            simulated_pos = (
                simulated_pos[0] + Command.COMMANDS[sample][0],
                simulated_pos[1] + Command.COMMANDS[sample][1],
            )
        return commands
    ...
    def reset(self, seed=None, return_info=True, options=None):
        ...
        self.max_episode_steps = calc_max_episode_steps(
            max(self.reset_params["command_count"]),
        ...
        spawn_pos = self.arena.get_tile_global_position(
            self.np_random.integers(0, self.reset_params["arena_size"] ** 2)
        )
        translate_x = (
            self.arena.rect.center[0]
            - self.arena.local_center[0]
        ...
        translate_y = (
            self.arena.rect.center[1]
            - self.arena.local_center[1]
            + self.arena.tile_dim // 2
        )
        ...
        self.agent = GridCharacterController(
            SCALE, self.normalized_agent_position, self.arena.to_grid()
        )

        # Sample the entire command sequence
        self._commands = self._generate_commands(self.normalized_agent_position)
        show_duration = self.np_random.choice(
            self.reset_params["command_show_duration"]
        )
        ...
        self._command_visualization = self._generate_command_visualization(
            self._commands, show_duration, show_delay
        )
        self._command_visualization_clone = (
            self._command_visualization.copy()
        )  # the clone is needed for render()
        # Retrieve the first command frame
        command = Command(self._command_visualization.pop(0), SCALE)

        # Init episode members
        self._target_pos = (
            self.normalized_agent_position[0] + Command.COMMANDS[self._commands[0]][0],
        ...
        self._command_steps = (
            0  # the current step while executing a command (i.e. death tiles off)
        )
        ...
        self._explosion_duration = self.np_random.choice(
            self.reset_params["explosion_duration"]
        )
        self._explosion_delay = self.np_random.choice(
            self.reset_params["explosion_delay"]
        )
        ...
        self._draw_surfaces(
            [
                (self.bg, (0, 0)),
                (self.arena.surface, self.arena.rect),
                self.agent.get_rotated_sprite(0),
                (
                    command.surface,
                    (
                        (
        ...
        vis_obs = pygame.surfarray.array3d(pygame.display.get_surface()).astype(
            np.uint8
        )  # pygame.surfarray.pixels3d(pygame.display.get_surface()).astype(np.uint8)
    ...
    def step(self, action):
        ...
        if self._command_visualization:
            command = Command(self._command_visualization.pop(0), SCALE)
            self.rotated_agent_surface, self.rotated_agent_rect = (
                self.agent.get_rotated_sprite(0)
            )
        # All commands were shown, the agent can move now, while the command execution logic is running
            ...
            self.rotated_agent_surface, self.rotated_agent_rect = self.agent.step(
                action
            )
            self.normalized_agent_position = self._normalize_agent_position(
                self.rotated_agent_rect.center
            )

            # Process the command execution logic
            # One command is alive for explosion delay steps
            verify = (
                self._command_steps % self._explosion_delay == 0
                and self._command_steps > 0
            )
            ...
            if verify and not self.arena.tiles_on:
                if self._current_command < self.num_commands:
                    ...
                    self.arena.toggle_tiles(
                        self._target_pos, self.reset_params["visual_feedback"]
                    )
            ...
            if self.arena.tiles_on:
                if (
                    self._command_verify_step % self._explosion_duration == 0
                    and self._command_verify_step > 0
                ):
                    ...
                    if self._current_command < self.num_commands:
                        ...
                        self._target_pos = (
                            self._target_pos[0]
                            + Command.COMMANDS[self._commands[self._current_command]][
                                0
                            ],
                            self._target_pos[1]
                            + Command.COMMANDS[self._commands[self._current_command]][
        ...
        if done:
            info = {
                ...
                "success": success,
                "commands_completed": (self._current_command - 1 + success)
                / self.num_commands,
            }
        else:
            info = {}
    ...
    def render(self):
        ...
        if self.render_mode == "human":
            ...
            debug_surface = self._build_debug_surface()
            return np.fliplr(
                np.rot90(pygame.surfarray.array3d(debug_surface).astype(np.uint8), 3)
            )
...
def main():
    ...
    parser.add_argument(
        "--seed",
        type=int,
        help="The to be used seed for the environment's random number generator.",
        default=0,
    )
