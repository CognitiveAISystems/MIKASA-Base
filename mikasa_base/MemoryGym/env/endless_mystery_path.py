...
from .pygame_assets import EndlessMysteryPath, draw_column_tile_surface
...
class EndlessMysteryPathEnv(CustomEnv):
    ...
    def __init__(self, render_mode=None) -> None:
        ...
        self.tile_coloumn_surface = draw_column_tile_surface(
            self.tile_dim, self.grid_dim
        )
    ...
    def _draw_past_path(self):
        ...
        while x >= past_x and x >= 0:
            ...
            pygame.draw.rect(
                self.screen,
                (255, 255, 255),
                (draw_x, y * self.tile_dim, self.tile_dim, self.tile_dim),
            )
            # Draw border
            pygame.draw.rect(
                self.screen,
    ...
    def _draw_surfaces(self, surfaces):
        ...
        if self.reset_params["show_background"]:
            for i in range(self.num_coloums):
                self.screen.blit(
                    self.tile_coloumn_surface,
                    (i * self.tile_dim + self.bg_scroll - self.tile_dim, 0),
                )
        else:
        ...
        # Draw origin
        if (
            self.reset_params["show_origin"]
            and self.normalized_agent_position[0] < 5
            and not self.reset_params["show_origin"]
        ):
            pygame.draw.rect(
                self.screen,
                (0, 255, 0),
                (
                    self.start.x * self.tile_dim - self.camera_x,
                    self.start.y * self.tile_dim,
                    self.tile_dim,
                    self.tile_dim,
                ),
            )
    ...
    def _build_debug_surface(self):
        ...
        if self.rotated_agent_surface:
            surface.blit(
                self.rotated_agent_surface,
                (self.agent_draw_x, self.rotated_agent_rect.y),
            )
        else:
    ...
    def reset(self, seed=None, return_info=True, options=None):
        ...
        self.endless_path = EndlessMysteryPath(
            self.grid_dim,
            self.grid_dim,
            self.tile_dim,
            self.np_random,
            num_initial_segments=3,
        )
        ...
        self.agent = CharacterController(
            self.reset_params["agent_speed"], self.reset_params["agent_scale"], 270
        )
        self.rotated_agent_surface, self.rotated_agent_rect = (
            self.agent.get_rotated_sprite(270)
        )
        # Place the agent on the path's starting position
        self.agent.rect.center = (
            self.start.x * self.tile_dim + self.agent.radius,
            self.start.y * self.tile_dim + self.agent.radius,
        )
        ...
        height = int(
            ...
            * (
                1
                - (
                    max(self.stamina, self.max_visible_stamina)
                    / self.max_visible_stamina
                )
            )
        )
        ...
        if self.reset_params["show_stamina"]:
            self._draw_surfaces(
                [
                    ...
                    (self.stamina_surface, self.stamina_position),
                ]
            )
        else:
            self._draw_surfaces(
    ...
    def step(self, action):
        ...
        self.current_segment = (
            int(self.normalized_agent_position[0]) // self.endless_path.segment_length
        )
        ...
        for node in nodes:
            if self.normalized_agent_position == (node.x, node.y):
                ...
                if not node.reward_visited and not (node.x, node.y) == (
                    self.start.x,
                    self.start.y,
                ):
                ...
                if not node.stamina_visited and not (node.x, node.y) == (
                    self.start.x,
                    self.start.y,
                ):
        ...
        if not on_path:
            ...
            self.fall_off_surface.set_alpha(0)
            self.is_off_path = False
        self.fall_off_rect.center = (
            self.agent.rect.center[0] - self.camera_x,
            self.agent.rect.center[1],
        )
        ...
        height = int(
            ...
            * (
                1
                - (
                    min(self.stamina, self.max_visible_stamina)
                    / self.max_visible_stamina
                )
            )
        )
