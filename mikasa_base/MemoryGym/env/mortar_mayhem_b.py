...
from .mortar_mayhem import MortarMayhemEnv
...
class MortarMayhemTaskBEnv(MortarMayhemEnv):
    ...
    def process_reset_params(reset_params):
        ...
        assert (
            max(cloned_params["command_count"]) <= 20
        ), "20 commands are allowed at maximum"
    ...
    def reset(self, seed=None, return_info=True, options=None):
        ...
        return {
            ...
            "vector_observation": self._commands_one_hot,
        }, {}
    ...
    def step(self, action):
        ...
        verify = (
            self._command_steps % self._explosion_delay == 0 and self._command_steps > 0
        )
        ...
        if self.arena.tiles_on:
            if (
                ...
                if self._current_command < self.num_commands:
                    ...
                    self._target_pos = (
                        self._target_pos[0]
                        + Command.COMMANDS[self._commands[self._current_command]][0],
                        self._target_pos[1]
                        + Command.COMMANDS[self._commands[self._current_command]][1],
                    )
            else:
