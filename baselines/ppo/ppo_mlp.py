...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if len(envs.single_observation_space.shape) > 1:
            ...
            self.encode = 'state'
            self.encoder = layer_init(nn.Linear(envs.single_observation_space.shape[0], args.head_dim))
...
def evaluate(args, agent, envs, device, deterministic = False):
    print("Evaluating")
    with torch.no_grad():
...
def train(args = None):
    ...
    run_name = f"ppo_mlp_{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 0 and False:
